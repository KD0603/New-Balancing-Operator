...
Once actual data is known, each household's deviation is settled in order:
self-offset against own day-ahead unmatched -> internal P2P matching -> real grid trade.
"""
...
def split_amount_equally(rows, total_amount):
    ...
    priority_order = sorted(
        active_rows,
        key=lambda x: (_get_bonus_priority(x["household_id"]), x["household_id"]),
    )
...
# Offset the deviation against the unmatched part in the forecasting
def apply_per_household_unmatched_offset(row, own_unmatched_buy, own_unmatched_sell):
    if row["deviation_type"] == "surplus" and quantize(row["remaining_kwh"]) > 0:
        absorb = quantize(min(quantize(row["remaining_kwh"]), quantize(own_unmatched_buy)))
        if absorb > 0:
            row["unmatched_offset_kwh"] = round6(row.get("unmatched_offset_kwh", 0) + absorb)
            row["remaining_kwh"] = round6(row["remaining_kwh"] - absorb)
            row["detail_list"].append(f"self_offset_against_own_unmatched_buy: {absorb:.4f} kWh (own day-ahead forecast absorbed)")

    elif row["deviation_type"] == "shortage" and quantize(row["remaining_kwh"]) > 0:
        absorb = quantize(min(quantize(row["remaining_kwh"]), quantize(own_unmatched_sell)))
        if absorb > 0:
            row["unmatched_offset_kwh"] = round6(row.get("unmatched_offset_kwh", 0) + absorb)
            row["remaining_kwh"] = round6(row["remaining_kwh"] - absorb)
            row["detail_list"].append(f"self_offset_against_own_unmatched_sell: {absorb:.4f} kWh (own day-ahead forecast absorbed)")
    ...
    for timeslot, household_ids in grouped.items():
        ...
        for household_id in household_ids:
            ...
            deviation_settle = {
                ...
                "unmatched_offset_kwh": 0.0,
                "net_deviation_after_offset_kwh": 0.0,
            ...
            apply_per_household_unmatched_offset(deviation_settle, unmatched_buy, unmatched_sell)

            deviation_settle["net_deviation_after_offset_kwh"] = round6(deviation_settle["deviation_order_kwh"] - deviation_settle["unmatched_offset_kwh"])
        ...
        surplus_rows = [r for r in rows if r["deviation_type"] == "surplus"  and quantize(r["remaining_kwh"]) > 0]
        shortage_rows = [r for r in rows if r["deviation_type"] == "shortage" and quantize(r["remaining_kwh"]) > 0]
        ...
        for deviation_settle in rows:
            ...
            if deviation_settle["deviation_type"] == "surplus":
                deviation_settle["deviation_net_amount"] = round6(deviation_settle["internal_trade_amount"] + deviation_settle["final_grid_amount"])
